...
public class AuthenticationService {
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        LogUtils.logMethodInfo("Signer key: " + SIGNER_KEY);
    ...
    public String generateToken(User user) {
        ...
        JWTClaimsSet jwtClaimsSet = new JWTClaimsSet.Builder()
                ...
                .expirationTime(
                        new Date( // Thời gian hết hạn token
                                Instant.now()
                ...
                .claim(
                        "scope",
                        buildScope(user)) // Thông tin về phạm vi (scope) của token. Cho biết quyền hạn (roles) của user
                // đang sở hữu token này.
                .build();
    ...
    public String buildScope(User user) {
        StringJoiner stringJoiner =
                new StringJoiner(" "); // Vì theo chuẩn OAuth2, các scope của 1 user được phân tách bằng dấu cách
        if (!CollectionUtils.isEmpty(
                user.getRoles())) { // Kiểm tra xem user có role không. Nếu role của user không rỗng
            user.getRoles()
                    .forEach(
                            role -> { // Duyệt qua từng role của user
                                stringJoiner.add(
                                        "ROLE_" + role.getName()); // Thêm tên role vào chuỗi scope và thêm chuỗi
                                // "ROLE_" vào trước tên role để phân biệt role với
                                // permission
                                if (!CollectionUtils.isEmpty(
                                        role.getPermissions())) { // Kiểm tra xem role có permission không. Nếu
                                    // permission của role không rỗng
                                    role.getPermissions().forEach(permission -> {
                                        stringJoiner.add(permission.getName());
                                    }); // Duyệt qua từng permission của role và thêm tên permission vào chuỗi scope
                                }
                            });
        }
    ...
    private SignedJWT verifyToken(String token, boolean isRefresh) throws ParseException, JOSEException {
        ...
        Date expiryTime = (isRefresh) // nếu là làm mới token
                ? new Date(signedJWT
                        ...
                        .toEpochMilli()) // Thời gian hết hạn bằng thời gian phát hành + thời gian làm mới
                : signedJWT
                        .getJWTClaimsSet()
                        .getExpirationTime(); // Ngược lại, thời gian hết hạn bằng thời gian hết hạn trong token
        ...
        // Nếu token đã bị thu hồi hoặc user giữ token này logout trước khi nó expire, ném ra ngoại lệ là user chưa được
        // xác thực
