...
public class UserService {
    ...
    //    @PreAuthorize("hasAuthority('ROLE_ADMIN')") // Chỉ cho phép truy cập nếu người dùng có vai trò ADMIN. Sẽ kiểm
    // tra trước khi thực thi method này, nếu không đủ quyền sẽ ném lỗi 403 Forbidden ngay trước khi vào method này.
    @PreAuthorize("hasAuthority('APPROVE_POST')") // Xét bằng permission thay vì role
    // Trong video số 12. thì Devteria dùng là @PreAuthorize("hasRole('ADMIN')") do video trước đó ảnh có customize cái
    // prefix "ROLE_" nên mới dùng hasRole('ADMIN'), còn ở đây mình dùng mặc định của Spring Security nên phải dùng
    // hasAuthority('SCOPE_ADMIN')
    public List<UserResponse> getUsers() {
        LogUtils.logMethodInfo("Fetching all users");
        return userRepository.findAll().stream()
                .map(userMapper::toUserResponse)
                .collect(Collectors.toList()); // Sử dụng method reference để chuyển đổi từng User sang UserResponse
    }

    // Lấy thông tin người dùng theo ID
    //    @PostAuthorize("hasAuthority('SCOPE_ADMIN')") // Chỉ cho phép truy cập nếu người dùng có vai trò ADMIN. Sẽ
    // kiểm tra sau khi thực thi method này, nếu không đủ quyền sẽ ném lỗi 403 Forbidden ngay sau khi vào method này.
    @PostAuthorize(
            "returnObject.username == authentication.name") // Chỉ cho phép truy cập nếu username trong UserResponse
    // trùng với tên người dùng đã xác thực. Sẽ kiểm tra sau khi
    // thực thi method này, nếu không đủ quyền sẽ ném lỗi 403
    // Forbidden ngay sau khi vào method này.
